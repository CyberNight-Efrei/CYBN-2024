...
# On enlève les blocs chiants/instables au-dessus du vide ou sans support
BLOCKS = {
    "0": "minecraft:air",
    "1": "minecraft:stone",
    "2": "minecraft:grass",
...
def nearest_valid_ID(ID: int):
    keys = list(BLOCKS.keys())
    dist = 0
    while True:
        if str(ID - dist) in keys:
            return str(ID - dist)
        if str(ID + dist) in keys:
            return str(ID + dist)
        dist += 1
...
for x in range(img.width):
    for z in range(img.height):
        rgb = img.getpixel((x, z))
        mcIds = [nearest_valid_ID(c) for c in rgb]

        for offset in range(3):
            region.set_block(anvil.Block(*BLOCKS[mcIds[offset]].split(":")), x, 0, z * 3 + offset)

region.save("out/r.0.0.mca")